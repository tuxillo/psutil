...
psutil_net_connections(PyObject* self, PyObject* args) {
	...
	if (psutil_gather_inet(IPPROTO_TCP, py_retlist) == 0) {
		psutil_debug("Failed to gather inet tcp stats");
		goto error;
	}
	if (psutil_gather_inet(IPPROTO_UDP, py_retlist) == 0) {
		psutil_debug("Failed to gather inet udp stats");
		goto error;
	}
	if (psutil_gather_unix(SOCK_STREAM, py_retlist) == 0) {
		psutil_debug("Failed to gather inet stream stats");
		goto error;
	}
	if (psutil_gather_unix(SOCK_DGRAM, py_retlist) == 0) {
		psutil_debug("Failed to gather inet dgram stats");
		goto error;
	}
